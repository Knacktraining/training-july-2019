...
public class DefaultRentalDao extends AbstractItemDao implements RentalDao {
	...
	public List<RentalModel> getActiveRentalsForCustomer(final CustomerModel customer) {
		...
		*/
		 ...
		 *
		 * SELECT {rental.pk} FROM {Rental as rental} WHERE {rental.startDate} <= ?tomorrow" AND {rental.endDate} >=
		 * ?today AND {rental.customer} = ?customer
		 *
		 ...
		 * "WHERE {rental.startDate} <= ?tomorrow " + "AND {rental.endDate} >= ?today " +
		...
		final String queryString = "SELECT {rental.pk}" + " FROM {" + RentalModel._TYPECODE + " as rental}" + " WHERE {rental."
				+ RentalModel.STARTDATE + "} <= ?tomorrow" + " and {rental." + RentalModel.ENDDATE + "} >= ?today" + " and {rental."
